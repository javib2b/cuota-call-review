...
const FETCH_TIMEOUT_MS = 8000; // 8s per Supabase REST call

// AbortController-based fetch timeout (more reliable than AbortSignal.timeout in some runtimes)
function fetchWithTimeout(url, options, ms) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(new Error(`Supabase fetch timed out after ${ms}ms`)), ms);
...
export async function authenticateUser(token) {
  ...
  const pr = await fetchWithTimeout(`${SUPABASE_URL}/rest/v1/profiles?select=*&id=eq.${user.id}`, {
...
export function adminTable(table) {
  return {
    async select(query = "*", filters = "") {
      ...
      const r = await fetchWithTimeout(url, { headers: adminHeaders() }, FETCH_TIMEOUT_MS);
    ...
    async update(data, filters) {
      const r = await fetchWithTimeout(`${SUPABASE_URL}/rest/v1/${table}?${filters}`, {
        method: "PATCH",
        headers: { ...adminHeaders(), Prefer: "return=representation" },
        body: JSON.stringify(data),
      }, FETCH_TIMEOUT_MS);
