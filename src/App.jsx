...
function CategoryBar({ category, scores, onScoreChange }) {
  ...
  return (
    <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid rgba(0,0,0,0.08)", padding: "10px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#012441", lineHeight: 1.3 }}>{category.name}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
          <button onClick={() => onScoreChange(category.id, Math.max(0, score - 1))} style={{ width: 24, height: 24, border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, background: "#FFFFFF", cursor: "pointer", fontSize: 14, fontWeight: 700, color: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit" }}>−</button>
          <span style={{ width: 26, textAlign: "center", fontSize: 13, fontWeight: 700, color, fontFamily: "'Space Mono', monospace" }}>{score}</span>
          <button onClick={() => onScoreChange(category.id, Math.min(10, score + 1))} style={{ width: 24, height: 24, border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, background: "#FFFFFF", cursor: "pointer", fontSize: 14, fontWeight: 700, color: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit" }}>+</button>
        </div>
      </div>
        ...
        {archivedClients && archivedClients.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <button onClick={() => setShowArchived(v => !v)} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", padding: "4px 0", marginBottom: showArchived ? 12 : 0, fontFamily: "inherit" }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.25)", textTransform: "uppercase", letterSpacing: 1 }}>Archived Clients</span>
              <span style={{ fontSize: 11, padding: "1px 7px", borderRadius: 10, background: "rgba(0,0,0,0.05)", color: "rgba(0,0,0,0.3)", fontWeight: 600 }}>{archivedClients.length}</span>
              <span style={{ fontSize: 11, color: "rgba(0,0,0,0.2)", marginLeft: 2 }}>{showArchived ? "▲" : "▼"}</span>
            </button>
            {showArchived && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 }}>
                    ...
                    <div key={client} style={{ position: "relative", background: "#f9f9f9", border: "1px solid rgba(0,0,0,0.04)", borderRadius: 16, padding: 0, cursor: callCount === 0 ? "default" : "pointer", overflow: "hidden", opacity: 0.45, transition: "all 0.2s" }} onClick={() => callCount > 0 && (onClientClick ? onClientClick(client) : setFolderClient(client))}>
                      ...
                      <div style={{ padding: "20px 20px 14px", display: "flex", alignItems: "center", gap: 14 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 12, color: "rgba(0,0,0,0.2)", marginTop: 2 }}>{callCount > 0 ? `${callCount} call${callCount !== 1 ? "s" : ""}` : "No calls"}</div>
                        </div>
                      </div>
                    </div>
                  );
...
function ScoreTrendsChart({ repEntries }) {
  ...
  const [isolatedRep, setIsolatedRep] = useState(null);
  const [tooltip, setTooltip] = useState(null);

  // Build per-rep data: deduplicate by calendar date (highest score per day)
  const allRepData = repEntries.map((entry) => {
    const byDay = new Map();
    entry.repCalls
      .filter(c => c.call_date || c.created_at)
      .forEach(c => {
        const dateStr = (c.call_date || c.created_at || "").slice(0, 10);
        const score = c.overall_score || 0;
        if (!byDay.has(dateStr) || score > byDay.get(dateStr).score) {
          byDay.set(dateStr, {
    ...
    const pts = Array.from(byDay.values()).sort((a, b) => a.date - b.date);
    const avgScore = pts.length > 0 ? Math.round(pts.reduce((s, p) => s + p.score, 0) / pts.length) : 0;
    return { repName: entry.repName, pts, avgScore };
  }).filter(e => e.pts.length > 0);

  if (allRepData.length === 0) return null;

  // Filter subset and assign palette colors
  const sorted = [...allRepData].sort((a, b) => b.avgScore - a.avgScore);
  const subset = filter === "top5" ? sorted.slice(0, 5)
    : filter === "bottom5" ? sorted.slice(-5).reverse()
    : allRepData;
  const repData = subset.map((e, i) => ({ ...e, color: CHART_PALETTE[i % CHART_PALETTE.length] }));

  // Date-based X axis (fixes same-date stacking)
  const allTs = repData.flatMap(e => e.pts.map(p => p.date.getTime()));
  const minT = Math.min(...allTs);
  const maxT = Math.max(...allTs);
  const dateSpan = maxT - minT || 86400000;

  const PAD_L = 36, PAD_R = 70, PAD_T = 14, PAD_B = 34;
  const W = 760, H = 260;
  ...
  repData.forEach(e => e.pts.forEach(p => {
    const k = p.date.toISOString().slice(0, 10);
    if (!dayMap.has(k)) dayMap.set(k, { date: p.date, scores: [] });
    dayMap.get(k).scores.push(p.score);
  }));
  const teamAvgPts = Array.from(dayMap.values())
    .sort((a, b) => a.date - b.date)
    .map(({ date, scores }) => ({ date, score: Math.round(scores.reduce((s, v) => s + v, 0) / scores.length) }));

  // X-axis labels (up to 7)
  const allDateStrs = Array.from(new Set(repData.flatMap(e => e.pts.map(p => p.date.toISOString().slice(0, 10))))).sort();
  const allUniqueDates = allDateStrs.map(d => new Date(d + "T12:00:00"));
  const step = Math.max(1, Math.floor(allUniqueDates.length / 7));
  const xLabels = allUniqueDates.filter((_, i) => i % step === 0 || i === allUniqueDates.length - 1);

  // Interaction helpers
  const activeRep = isolatedRep || hoveredRep;
  const getOpacity = (rep) => activeRep ? (rep === activeRep ? 1 : 0.12) : 1;
  const getWidth = (rep) => rep === activeRep ? 3 : 2;

  // Inline legend box dimensions
  const LW = 118, LH = repData.length * 17 + 10;
  const LX = PAD_L + chartW - LW - 4, LY = PAD_T + 2;

  return (
    <div style={{ background: "#fff", borderTop: "1px solid rgba(0,0,0,0.08)", borderBottom: "1px solid rgba(0,0,0,0.08)", overflow: "hidden", marginBottom: 16, marginLeft: -40, marginRight: -40 }}>
      {/* Header + toolbar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, padding: "18px 40px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ display: "flex", background: "rgba(0,0,0,0.05)", borderRadius: 7, padding: 2 }}>
            {[["all", "All"], ["top5", "Top 5"], ["bottom5", "Bot 5"]].map(([v, l]) => (
              <button key={v} onClick={() => { setFilter(v); setIsolatedRep(null); }}
                style={{ fontSize: 11, fontWeight: filter === v ? 700 : 500, padding: "3px 9px",
                  background: filter === v ? "#fff" : "transparent", border: "none", borderRadius: 5,
                  cursor: "pointer", fontFamily: "inherit",
                  color: filter === v ? "#1A2B3C" : "rgba(0,0,0,0.4)",
          ...
          <button onClick={() => setShowTeamAvg(v => !v)}
            style={{ fontSize: 11, fontWeight: showTeamAvg ? 600 : 400, padding: "3px 10px",
              background: showTeamAvg ? "rgba(148,163,184,0.12)" : "transparent",
              border: `1px solid ${showTeamAvg ? "#94a3b8" : "rgba(0,0,0,0.12)"}`,
              borderRadius: 20, cursor: "pointer", fontFamily: "inherit",
              color: showTeamAvg ? "#64748b" : "rgba(0,0,0,0.35)", transition: "all 0.15s" }}>
            — Team Avg
          </button>
        </div>
      </div>

      <div style={{ position: "relative" }} onMouseLeave={() => { setHoveredRep(null); setTooltip(null); }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: 260, display: "block" }}>
          <defs>
            <style>{`@keyframes lineDrawIn{from{stroke-dashoffset:1}to{stroke-dashoffset:0}}`}</style>
          </defs>
          ...
          {showTeamAvg && teamAvgPts.length >= 2 && (() => {
            const d = `M ${teamAvgPts.map(p => `${toX(p.date)},${toY(p.score)}`).join(" L ")}`;
            const last = teamAvgPts[teamAvgPts.length - 1];
            return (
              <g opacity={activeRep ? 0.25 : 0.75}>
                <path d={d} fill="none" stroke="#94a3b8" strokeWidth={1.5} strokeDasharray="5 3" strokeLinecap="round" />
                <text x={toX(last.date) + 5} y={toY(last.score) + 4} fontSize={9} fill="#94a3b8" fontWeight="600">Avg</text>
              </g>
            );
          })()}

          {/* Invisible wide paths for line hover hit area */}
          {repData.map(e => {
            if (e.pts.length < 2) return null;
            return (
              <path key={`hit-${e.repName}`}
                d={`M ${e.pts.map(p => `${toX(p.date)},${toY(p.score)}`).join(" L ")}`}
                fill="none" stroke="transparent" strokeWidth={14}
                style={{ cursor: "pointer" }}
                onMouseEnter={() => setHoveredRep(e.repName)} />
            );
          })}

          {/* Rep lines with draw-in animation */}
          {repData.map((e, repIdx) => {
            if (e.pts.length < 2) return null;
            return (
              <path key={`line-${e.repName}`}
                d={`M ${e.pts.map(p => `${toX(p.date)},${toY(p.score)}`).join(" L ")}`}
                fill="none" stroke={e.color} strokeLinecap="round" strokeLinejoin="round"
                pathLength="1" strokeDasharray="1"
                style={{
                  strokeWidth: getWidth(e.repName),
                  opacity: getOpacity(e.repName),
                  animation: `lineDrawIn 800ms ease-out ${repIdx * 80}ms both`,
          ...
          {/* Dots */}
          {repData.map(e => e.pts.map((p, ptIdx) => {
            const cx = toX(p.date);
            const cy = toY(p.score);
            const active = tooltip?.rep === e.repName && tooltip?.ptIndex === ptIdx;
            const delta = ptIdx > 0 ? p.score - e.pts[ptIdx - 1].score : null;
            return (
              <circle key={`dot-${e.repName}-${ptIdx}`}
                cx={cx} cy={cy} r={active ? 5 : 3}
                fill={e.color} stroke="#fff" strokeWidth={active ? 2 : 1.5}
                style={{ cursor: "pointer", opacity: getOpacity(e.repName), transition: "opacity 0.2s ease" }}
                onMouseEnter={(ev) => {
                  const rect = ev.currentTarget.closest("svg").getBoundingClientRect();
                  setHoveredRep(e.repName);
                  setTooltip({
                    x: cx * (rect.width / W),
                    y: cy * (rect.height / H),
                    rep: e.repName, score: p.score, color: e.color,
                    callType: p.callType, delta, ptIndex: ptIdx,
          ...
          {/* Inline interactive legend (top-right of chart area) */}
          <g>
            <rect x={LX} y={LY} width={LW} height={LH} rx={7}
              fill="rgba(255,255,255,0.93)" stroke="rgba(0,0,0,0.07)" strokeWidth={1} />
            {repData.map((e, idx) => {
              const iy = LY + 9 + idx * 17;
              const isIso = isolatedRep === e.repName;
              return (
                <g key={`leg-${e.repName}`}
                  onClick={() => setIsolatedRep(p => p === e.repName ? null : e.repName)}
                  style={{ cursor: "pointer" }}
                  opacity={isolatedRep && !isIso ? 0.3 : 1}>
                  <circle cx={LX + 10} cy={iy} r={4} fill={e.color} />
                  <text x={LX + 18} y={iy + 3.5} fontSize={10} fill="#374151"
                    fontWeight={isIso ? "700" : "400"}
                    style={{ userSelect: "none" }}>
                    {e.repName.length > 13 ? e.repName.slice(0, 12) + "…" : e.repName}
                  </text>
                </g>
              );
            })}
          ...
          {/* X-axis date labels */}
          {xLabels.map((date, i) => (
            <text key={i} x={toX(date)} y={H - 6} textAnchor="middle" fontSize={9} fill="rgba(0,0,0,0.35)">
        ...
        {tooltip && (
          <div style={{
            position: "absolute",
            left: Math.min(tooltip.x + 14, 270), top: Math.max(4, tooltip.y - 72),
            background: "#fff", border: "1px solid rgba(0,0,0,0.09)",
            borderRadius: 10, padding: "9px 13px", fontSize: 11,
            pointerEvents: "none", zIndex: 20, whiteSpace: "nowrap",
            boxShadow: "0 4px 20px rgba(0,0,0,0.12)",
          }}>
            <div style={{ fontWeight: 700, color: "#1A2B3C", marginBottom: 5 }}>{tooltip.rep}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
              <span style={{ background: tooltip.color, color: "#fff", fontWeight: 700, fontSize: 12, padding: "2px 8px", borderRadius: 5 }}>{tooltip.score}%</span>
              {tooltip.delta !== null && (
                <span style={{ fontSize: 11, fontWeight: 600, color: tooltip.delta > 0 ? "#10B981" : tooltip.delta < 0 ? "#EF4444" : "#94a3b8" }}>
                  {tooltip.delta > 0 ? "▲" : tooltip.delta < 0 ? "▼" : "→"} {Math.abs(tooltip.delta)}
                </span>
              )}
            </div>
            <div style={{ color: "rgba(0,0,0,0.4)", fontSize: 10 }}>{tooltip.callType} · {tooltip.date}</div>
          </div>
        )}
      </div>
    </div>
...
function ClientIntelPage({ getValidToken, clients }) {
  ...
  const inputStyle = { width: "100%", padding: "8px 10px", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 8, fontSize: 13, color: "#012441", outline: "none", boxSizing: "border-box", fontFamily: "inherit", background: "#FFFFFF" };
  const labelStyle = { fontSize: 10, textTransform: "uppercase", letterSpacing: 1.2, color: "rgba(0,0,0,0.35)", display: "block", marginBottom: 5, fontWeight: 700 };
  const cardStyle = { background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: "16px 18px" };
  const sectionLabelStyle = { fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(0,0,0,0.3)", marginBottom: 14 };

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#012441", margin: 0 }}>Client Intel</h1>
        <p style={{ fontSize: 13, color: "rgba(0,0,0,0.4)", margin: "4px 0 0" }}>AI-powered company research and deal intelligence</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, alignItems: "start" }}>
        ...
        <div>
          {!results && !loading && (
            <div style={{ border: "1px dashed rgba(0,0,0,0.12)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "rgba(0,0,0,0.01)" }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>🔍</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: "#012441", marginBottom: 6 }}>Enter a company to research</div>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.35)" }}>AI generates company overview, pain points, competitive landscape, and deal prep</div>
            </div>
          )}
          {loading && (
            <div style={{ border: "1px solid rgba(0,0,0,0.08)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "#FFFFFF" }}>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.45)", marginBottom: 6 }}>Researching {company}\u2026</div>
              <div style={{ fontSize: 11, color: "rgba(0,0,0,0.25)" }}>Usually takes 10\u201315 seconds</div>
            </div>
          )}
          ...
          {results && !results.error && (
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: "#012441" }}>{company}</div>
                  <div style={{ fontSize: 12, color: "rgba(0,0,0,0.4)", marginTop: 2 }}>Intelligence Report</div>
                </div>
                <button onClick={() => { setResults(null); setCompany(""); setContext(""); }} style={{ fontSize: 11, padding: "6px 12px", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 8, background: "#FFFFFF", cursor: "pointer", color: "rgba(0,0,0,0.4)", fontFamily: "inherit" }}>Clear</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {sections.map(sec => {
                  ...
                  return (
                    <div key={sec.key} style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, overflow: "hidden" }}>
                      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(1,36,65,0.03)" }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "#012441", textTransform: "uppercase", letterSpacing: 1 }}>{sec.label}</span>
                      </div>
                      <div style={{ padding: "12px 16px", fontSize: 12, color: "rgba(0,0,0,0.65)", lineHeight: 1.65, maxHeight: 200, overflowY: "auto", whiteSpace: "pre-wrap", wordWrap: "break-word" }}>{content}</div>
                    </div>
                  );
                })}
              </div>
...
function CuotaAgentPage({ getValidToken, savedCalls }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#012441", margin: 0 }}>Cuota Agent</h1>
        <p style={{ fontSize: 13, color: "rgba(0,0,0,0.4)", margin: "4px 0 0" }}>AI-powered sales call analysis using the C.U.O.T.A. framework</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, alignItems: "start" }}>
        {/* LEFT PANEL */}
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {savedCalls.length > 0 && (
            <div style={cardStyle}>
              ...
              {savedCalls.slice(0, 5).map((call, i) => (
                <button key={i} onClick={() => { setTranscript(call.transcript || ""); setResults(null); }} style={{ display: "block", width: "100%", padding: "8px 10px", marginBottom: 6, border: "1px solid rgba(0,0,0,0.07)", borderRadius: 8, background: "#FFFFFF", cursor: "pointer", fontSize: 12, color: "#012441", textAlign: "left", fontFamily: "inherit" }}>
        ...
        <div>
          {!results && !loading && (
            <div style={{ border: "1px dashed rgba(0,0,0,0.12)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "rgba(0,0,0,0.01)" }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>🤖</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: "#012441", marginBottom: 6 }}>Paste a transcript to begin</div>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.35)" }}>Agent scores the call on C.U.O.T.A. and surfaces coaching insights</div>
            </div>
          )}
          {loading && (
            <div style={{ border: "1px solid rgba(0,0,0,0.08)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "#FFFFFF" }}>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.45)", marginBottom: 6 }}>Analyzing your call\u2026</div>
              <div style={{ fontSize: 11, color: "rgba(0,0,0,0.25)" }}>Usually takes 15\u201320 seconds</div>
            </div>
          )}
          {results && results.error && (
            <div style={{ padding: "16px 20px", background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 12, color: "#dc2626", fontSize: 13 }}>{results.error}</div>
          ...
          {results && !results.error && (
            <div>
              ...
              <div style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: "16px 20px", marginBottom: 16 }}>
                  ...
                  {Object.entries(frameColors).map(([frame, color]) => {
                    ...
                    return (
                      <div key={frame} style={{ textAlign: "center" }}>
                        ...
                        <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(0,0,0,0.4)", textTransform: "uppercase", letterSpacing: 1, margin: "5px 0 8px" }}>{frame}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[
                  ...
                  return (
                    <div key={section.label} style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 12, overflow: "hidden" }}>
                      <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(1,36,65,0.03)" }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "#012441", textTransform: "uppercase", letterSpacing: 1 }}>{section.label}</span>
                      </div>
                      <div style={{ padding: "12px 16px", fontSize: 12, color: "rgba(0,0,0,0.65)", lineHeight: 1.65, maxHeight: 220, overflowY: "auto", whiteSpace: "pre-wrap" }}>{content}</div>
                    </div>
                  );
                })}
              </div>
...
function PresentationBuilderPage({ clients, apiKey, getValidToken }) {
  const [brand, setBrand] = useState(() => loadStored("cuota_deck_brand") || {
    ...
    primaryColor: "#012441", secondaryColor: "#31CE81",
    companyName: "",
  });
  ...
  const inputStyle = { width: "100%", padding: "8px 10px", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 8, fontSize: 13, color: "#012441", outline: "none", boxSizing: "border-box", fontFamily: "inherit", background: "#FFFFFF" };
  const labelStyle = { fontSize: 10, textTransform: "uppercase", letterSpacing: 1.2, color: "rgba(0,0,0,0.35)", display: "block", marginBottom: 5 };
  const cardStyle = { background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: "16px 18px" };
  const sectionLabelStyle = { fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(0,0,0,0.3)", marginBottom: 14 };
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#012441", margin: 0 }}>Deck Builder</h1>
          <p style={{ fontSize: 13, color: "rgba(0,0,0,0.4)", margin: "4px 0 0" }}>AI-generated sales deck with your branding → export .pptx for Google Slides or Canva</p>
        </div>
        {slides && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div onClick={() => document.getElementById("deck-logo-input").click()} style={{ border: "1px dashed rgba(0,0,0,0.15)", borderRadius: 8, padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, background: "rgba(0,0,0,0.01)", minHeight: 44 }}>
                {brand.logoBase64
                  ? <><img src={brand.logoBase64} alt="logo" style={{ height: 28, maxWidth: 80, objectFit: "contain" }} /><span style={{ fontSize: 11, color: "rgba(0,0,0,0.4)" }}>{brand.logoFileName}</span><button onClick={e => { e.stopPropagation(); saveBrand({ logoBase64: null, logoFileName: null }); }} style={{ marginLeft: "auto", fontSize: 11, color: "rgba(0,0,0,0.3)", border: "none", background: "none", cursor: "pointer" }}>Remove</button></>
                  : <span style={{ fontSize: 12, color: "rgba(0,0,0,0.35)" }}>Click to upload logo (PNG / SVG / JPG)</span>
                }
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[{ key: "primaryColor", label: "Primary" }, { key: "secondaryColor", label: "Accent" }].map(c => (
                <div key={c.key}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <input type="color" value={brand[c.key]} onChange={e => saveBrand({ [c.key]: e.target.value })} style={{ width: 32, height: 32, border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, cursor: "pointer", padding: 2 }} />
                    <input value={brand[c.key]} onChange={e => saveBrand({ [c.key]: e.target.value })} style={{ flex: 1, padding: "6px 8px", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 6, fontSize: 12, color: "#012441", outline: "none", fontFamily: "monospace" }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={cardStyle}>
            ...
            {[
              ...
              <div key={f.key} style={{ marginBottom: 10 }}>
                ...
                {f.type === "select"
                  ? <select value={ctx[f.key]} onChange={e => setCtx(p => ({ ...p, [f.key]: e.target.value }))} style={{ ...inputStyle, color: ctx[f.key] ? "#012441" : "rgba(0,0,0,0.35)" }}>
          ...
          {/* Reference */}
          <div style={cardStyle}>
            <div style={sectionLabelStyle}>Reference Material <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: 10, color: "rgba(0,0,0,0.25)" }}>— optional</span></div>
            <p style={{ fontSize: 11, color: "rgba(0,0,0,0.35)", margin: "0 0 10px" }}>Upload a previous deck, brand copy, or case study for AI to match your style and messaging.</p>
          ...
          <button onClick={generateDeck} disabled={generating} style={{ padding: "13px 20px", background: generating ? "rgba(0,0,0,0.05)" : "#6366F1", border: "none", borderRadius: 12, color: generating ? "rgba(0,0,0,0.4)" : "#fff", fontSize: 14, fontWeight: 700, cursor: generating ? "wait" : "pointer", fontFamily: "inherit", width: "100%" }}>
        ...
        <div>
          {!slides && !generating && (
            <div style={{ border: "1px dashed rgba(0,0,0,0.12)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "rgba(0,0,0,0.01)" }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: "#012441", marginBottom: 6 }}>Your deck will appear here</div>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.35)" }}>Fill in the context on the left and click Generate Deck</div>
            </div>
          )}
          {generating && (
            <div style={{ border: "1px solid rgba(0,0,0,0.08)", borderRadius: 16, padding: "64px 40px", textAlign: "center", background: "#FFFFFF" }}>
              <div style={{ fontSize: 13, color: "rgba(0,0,0,0.45)", marginBottom: 6 }}>Building your personalized deck\u2026</div>
              <div style={{ fontSize: 11, color: "rgba(0,0,0,0.25)" }}>Usually takes 15–20 seconds</div>
            </div>
          )}
          {slides && (
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.35)", textTransform: "uppercase", letterSpacing: 1.2 }}>{slides.length} slides generated</span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
                {slides.map((slide, idx) => {
                  ...
                  return (
                    <div key={idx} style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.07)", borderRadius: 10, overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "10px 14px 6px" }}>
                        {(previewLines || []).slice(0, 3).map((line, i) => (
                          <div key={i} style={{ fontSize: 11, color: "rgba(0,0,0,0.5)", marginBottom: 4, lineHeight: 1.4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            <span style={{ color: brand.secondaryColor, marginRight: 6, fontWeight: 700, fontSize: 10 }}>·</span>
                            {typeof line === "string" ? line : JSON.stringify(line)}
                          </div>
                        ))}
                        {(previewLines?.length || 0) > 3 && <div style={{ fontSize: 10, color: "rgba(0,0,0,0.2)", marginTop: 2 }}>+{previewLines.length - 3} more</div>}
                      </div>
                      <div style={{ padding: "0 14px 8px" }}>
                        <span style={{ fontSize: 10, color: "rgba(0,0,0,0.18)", fontFamily: "monospace" }}>{idx + 1} / {slides.length}</span>
                      </div>
                    </div>
                  );
                })}
              </div>
              <p style={{ fontSize: 11, color: "rgba(0,0,0,0.3)", marginTop: 16, textAlign: "center" }}>
      ...
      <div style={{ marginLeft: 220, flex: 1, padding: "32px 40px" }}>
        ...
        {page === "review" && (
          <>
            ...
            {/* Score Hero */}
            <div style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: "12px 18px", marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(0,0,0,0.4)", fontFamily: "'Space Mono', monospace" }}>{totalRaw} / {activeCategories.length * 10} pts</div>
                  <div style={{ fontSize: 10, color: "rgba(0,0,0,0.3)", marginTop: 2 }}>{activeCategories.length} categories</div>
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: 0, marginBottom: 12, borderBottom: "1px solid rgba(0,0,0,0.08)", overflowX: "auto" }}>
              {tabs.map(tab => (
            ...
            {activeTab === "scorecard" && (
              <div>
                <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(0,0,0,0.3)", marginBottom: 8 }}>Category Scores</div>
